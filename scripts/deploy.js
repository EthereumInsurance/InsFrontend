...
const YOUR_ADDRESS = "0x46F25f07A08155A125C4697A1DB96485bF14685d";
...
async function main() {
  ...
  const [deployAccount] = await ethers.getSigners();
  console.log(`deployAccount address is ${deployAccount.address}`)

  const MINT_ADDRESS = "0x600103d518cc5e8f3319d532eb4e5c268d32e604";
  const MINT = await ethers.getContractAt("MockAave", MINT_ADDRESS);
  console.log(`mint address works`)

  // updateArtifactsFolder();
  // saveArtifactsToFrontend();
...
//   await daiForImp.transfer(deployAccount.address, parseEther("11000000"), { from: DAI_WHALE })
//
...
//
  // // Deploy contracts from deployAccount
...
//
//   // Also need to approve Dai from deployAccount to insurance contract
//   const defaultDai = await new ethers.Contract(daiAddress, daiABI, deployAccount);
//   await defaultDai.approve(insurance.address, ethers.constants.MaxUint256);
//   console.log(`allowance for insurance address from deployAccount is ${await defaultDai.allowance(deployAccount.address, insurance.address)}`)
//
//
...
//
//   // We are going to add funds to the pool from deployAccount's account
...
//   await insurance.getFunds(await deployAccount.getAddress());
//
...
async function saveArtifactsToFrontend() {

  const artifactSource = __dirname + "/../artifacts";
  const artifactDestination = __dirname + "/../frontend/src/artifacts";

  if (!fs.existsSync(artifactDestination)) {
    fs.mkdirSync(artifactDestination);
  }

  await fs.copy(artifactSource, artifactDestination).then(console.log("artifact files copied to frontend"));
